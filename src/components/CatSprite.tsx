...
export default function CatSprite() {
	const { state } = useContext(ScratchContext);

	return (
		<div id="cat-sprite" className="transition-all ease-linear relative">
			<svg
			...
			{(state.looksType == 'say' || state.looksType == 'say-time') && (
				<div className="absolute top-[-70px] left-[70px]">
					<div className="relative">
						<GiNothingToSay size={90} />
					</div>
					<p className="text-white absolute top-6 left-6">{state.message}</p>
				</div>
			)}
			{(state.looksType == 'think' || state.looksType == 'think-time') && (
				<div className="absolute top-[-70px] rounded-lg left-[70px] min-w-6 min-h-10 border-2 text-black border-solid p-4 border-slate-900 ">
